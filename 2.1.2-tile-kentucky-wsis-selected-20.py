# python3 2.1.2-tile-kentucky-wsis-selected-20.py --save-dir /home/bhavesh/ml-work/20imgs/tiles1 --nproc 25
...
def parse_args():
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="/home/bhavesh/ml-work/20imgs/tiles1",
...
def main(args):
    ...
    # Fetch first 20 annotated items
    annotations = {}
    items = list(gc.listItem(args.dsa_folder_id))[:20]
    for item in tqdm(items, desc="Reading annotations from DSA (max 20):"):
        ann_docs = []
    ...
    for i, (item_name, ann_docs) in enumerate(list(annotations.items())[:20]):
        print(f"({i + 1}/20) Processing {item_name}")
